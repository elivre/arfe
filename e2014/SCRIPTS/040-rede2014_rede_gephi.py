...
mtse.execute_query(f"""
                    update {table_gephi_nodes} 
                     set id = 'CA'||ue||nr_candidato
                     where 'CA'||ue||nr_candidato not in 
                         (select id from {table_gephi_nodes}  )
                         and id = 'CD000000000-4'
...
query_update_ids = f"""
    ...
    update {table_gephi_nodes}
    set id    = c.candidatura_id,
        label = c.candidatura_label,
        tipo  = 'CA',
        nome  = c.candidatura_nome
    from candidatos as c
