...
table_despesas_candidatos = f"tse{ano_eleicao}.despesas_candidatos_{ano_eleicao}"
...
query_update_cnpj_a_partir_receitas = f"""
    ...
    SELECT 
      ...
      get_tse_id(sequencial_candidato) as tse_id 
    FROM 
    ...
    update {table_candidaturas} as c
      set nr_cnpj_prestador_conta = r.cnpj_prestador_conta,
          declarou_receita = 'S',
...
query_update_cnpj_a_partir_despesas = f"""
    ...
    SELECT 
      sequencial_candidato, 
      cnpj_prestador_conta,
      round(sum(valor_despesa),2) as despesa_total,
      get_tse_id(sequencial_candidato) as tse_id  
    FROM 
    ...
    update {table_candidaturas} as c
      set nr_cnpj_prestador_conta = d.cnpj_prestador_conta,
          declarou_despesa = 'S',
...
query_atualiza_total_votos_turno_1 = f"""
    ...
    (
      select
       get_tse_id(sq_candidato) as tse_id,
       sum(total_votos::numeric) as total_votos
      from 
